...
async def signup(request: SignupRequest):
	...
	client=firebase.sign_up(email=request.email, password=request.password,name=request.name)
...
async def get_current(token:Token):
    ...
    my_token=firebase.get_current_user(token.token)
    return {
		"message":my_token["message"]['localId'],
	}
